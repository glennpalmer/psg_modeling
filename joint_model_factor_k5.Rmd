...
ggcorrplot(corr_mat,
  ...
  scale_x_discrete(labels=unname(TeX(c(r"($\gamma_{RA}$)",
                                       r"($\alpha_{RA}$)",
                                       r"($\gamma_{RN}$)",
                                       r"($\alpha_{RN}$)",
                                       r"($\gamma_{NR}$)",
                                       ...
                                       r"($\phi_R$)",
                                       r"($\phi_N$)")))) +
  scale_y_discrete(labels=unname(TeX(c(r"($\gamma_{RA}$)",
                                       r"($\alpha_{RA}$)",
                                       r"($\gamma_{RN}$)",
                                       r"($\alpha_{RN}$)",
                                       r"($\gamma_{NR}$)",
...
ggplot(data=rate_main_df, aes(x=parameter, y=posterior_mean)) +
  ...
  scale_x_discrete(labels=unname(TeX(c(r"($\lambda_{N}$)",
                                       r"($\lambda_{R}$)"))))
...
ggplot(data=rate_re_var_df, aes(x=parameter, y=posterior_mean)) +
  ...
  theme(axis.text.x = element_text(size=16),
        axis.text.y = element_text(size=16)) +
  scale_x_discrete(labels=unname(TeX(c(r"(var($\phi_{N}$))",
                                       r"(var($\phi_{R}$))"))))
...
ggplot(data=maineffect_df, aes(x=parameter, y=posterior_mean)) +
  ...
  labs(title="Markov transition model fixed effects",
       ...
       x="", y="") +
  scale_x_discrete(labels=unname(TeX(c(r"($\tau_{RN}$)",
                                       r"($\tau_{RA}$)",
                                       r"($\tau_{NR}$)",
                                       r"($\tau_{N A}$)",
                                       r"($\mu_{RN}$)",
...
ggplot(data=rate_re_var_df, aes(x=parameter_label, y=posterior_mean)) +
  ...
  labs(title="Markov transition model random effect variances",
       ...
       x="", y="") +
  scale_x_discrete(labels=unname(TeX(c(r"(var($\alpha_{RN}$))",
                                       r"(var($\alpha_{RA}$))",
                                       r"(var($\alpha_{NR}$))",
                                       r"(var($\alpha_{N A}$))",
                                       r"(var($\gamma_{RN}$))",
...
```{r}
# make elbow plot to choose number of clusters -- looks like 4
...
for (k in 1:10) {
  kmeans_model <- kmeans(theta_postmean, centers = k)
  wcss_values <- c(wcss_values, kmeans_model$tot.withinss)
}

# Create the elbow plot
plot(1:10, wcss_values, type = "b",
