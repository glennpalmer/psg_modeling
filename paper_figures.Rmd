...
```{r}
# add the alt clusters to the data
...
# Regression of pathfinder on clusters

```{r}
df_pathfinder <- df_figures |>
  filter(PFNTOTL > 0)

df_pathfinder$cluster <- relevel(df_pathfinder$cluster, ref="4")
```

```{r}
# outcome 
Pathfinder_score <- df_pathfinder$PFNTOTL

# covariates
age <- df_pathfinder$age_at_enrollment
isMale <- df_pathfinder$isMale
cluster1 <- (df_pathfinder$cluster == "1")
cluster2 <- (df_pathfinder$cluster == "2")
cluster3 <- (df_pathfinder$cluster == "3")
AHI <- df_pathfinder$AHI
```


Fit model for Pathfinder regressed on clusters:
...
```{r}
# load stan model
pathfinder_model_cluster <-
  stan_model("bsrt_cluster_linreg.stan")

# fit model

N <- length(Pathfinder_score)
coef_prior_sd <- 100;
options(mc.cores = parallel::detectCores())
pathfinder_fit <- sampling(bsrt_model_cluster,
                       list(N=N,
                            coef_prior_sd=coef_prior_sd,
                            bsrt=Pathfinder_score,
                            age=age,
                            sexMale=isMale,
                            cluster1=cluster1,
                            cluster2=cluster2,
...
```{r}
# extract posterior samples and summarize
post_samples_pathfinder <- extract(pathfinder_fit)
```

```{r}
# posterior means
mean(post_samples_pathfinder$intercept)
mean(post_samples_pathfinder$beta_cluster1)
mean(post_samples_pathfinder$beta_cluster2)
mean(post_samples_pathfinder$beta_cluster3)
mean(post_samples_pathfinder$beta_age)
mean(post_samples_pathfinder$beta_sexMale)
```
```{r}
# 95% credible intervals
quantile(post_samples_pathfinder$intercept, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_cluster1, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_cluster2, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_cluster3, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_age, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_sexMale, probs=c(0.025,0.975))
```


And for Pathfinder regressed on AHI.
...
coef_prior_sd <- 100
options(mc.cores = parallel::detectCores())
pathfinder_fit_ahi <- sampling(bsrt_model_ahi,
                       list(N=N,
                            coef_prior_sd=coef_prior_sd,
                            bsrt=Pathfinder_score,
                            age=age,
...
```{r}
# extract posterior samples and summarize
post_samples_pathfinder <- extract(pathfinder_fit_ahi)
```

```{r}
# posterior means
mean(post_samples_pathfinder$intercept)
mean(post_samples_pathfinder$beta_AHI)
mean(post_samples_pathfinder$beta_age)
mean(post_samples_pathfinder$beta_sexMale)
```
...
# 95% credible intervals
quantile(post_samples_pathfinder$intercept, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_AHI, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_age, probs=c(0.025,0.975))
quantile(post_samples_pathfinder$beta_sexMale, probs=c(0.025,0.975))
```
...
age <- df_pathfinder$age_at_enrollment
isMale <- df_pathfinder$isMale
cluster1_alt <- (df_pathfinder$alt_cluster_labels == "1")
cluster2_alt <- (df_pathfinder$alt_cluster_labels == "2")
cluster3_alt <- (df_pathfinder$alt_cluster_labels == "3")
AHI <- df_pathfinder$AHI
```
