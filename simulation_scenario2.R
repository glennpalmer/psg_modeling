###############################################################################
############ Script to run simulation scenario 2 on the server ################
###############################################################################
...
# fixed effect vector baseline (mu_NR, mu_NA, mu_RN, mu_RA, tau_NR, tau_NA, tau_RN, tau_RA, lambda_R, lambda_N)
fixed_vec_intercept <- rep(NA, 10)
fixed_vec_intercept[1:4] <- runif(n=4, min=-4, max=-3) # mu
fixed_vec_intercept[5:8] <- runif(n=4, min=0, max=1) # tau
fixed_vec_intercept[9:10] <- runif(n=2, min=0.01, max=0.03) # lambda

# allow event rates and baseline transition rates to change over time
fixed_vec_slope <- c(runif(n=4, min=-0.001, max=0.001), rep(0,4), runif(n=2, min=-0.00001, max=0.00001))

# create version of fixed vec for each epoch
fixed_vec_mat <- matrix(nrow=n_epochs, ncol=10)
for (i in 1:n_epochs) {
  fixed_vec_mat[i,] <- fixed_vec_intercept + i * fixed_vec_slope
}

# compute column means for reference later
fixed_vec_mean <- colMeans(fixed_vec_mat)
...
for (i in 1:n_patients) {
  ...
  while (curr_epoch <= n_epochs) {
    ######### Compute transition probabilities for the current epoch #######
    ...
    # REM, no event
    REM_transition_prob[1] <- exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4]) /
      (1 + exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4]) + exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3]))
    REM_transition_prob[2] <- 1 /
      (1 + exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4]) + exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3]))
    REM_transition_prob[3] <- exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3]) /
      (1 + exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4]) + exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3]))
    
    # REM, event
    REM_transition_prob_event[1] <- exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4] + fixed_vec_mat[curr_epoch,8] + theta_curr[8]) /
      (1 + exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4] + fixed_vec_mat[curr_epoch,8] + theta_curr[8]) +
         exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3] + fixed_vec_mat[curr_epoch,7] + theta_curr[7]))
    REM_transition_prob_event[2] <- 1 /
      (1 + exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4] + fixed_vec_mat[curr_epoch,8] + theta_curr[8]) +
         exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3] + fixed_vec_mat[curr_epoch,7] + theta_curr[7]))
    REM_transition_prob_event[3] <- exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3] + fixed_vec_mat[curr_epoch,7] + theta_curr[7]) /
      (1 + exp(fixed_vec_mat[curr_epoch,4] + theta_curr[4] + fixed_vec_mat[curr_epoch,8] + theta_curr[8]) +
         exp(fixed_vec_mat[curr_epoch,3] + theta_curr[3] + fixed_vec_mat[curr_epoch,7] + theta_curr[7]))
    
    # nonREM, no event
    nonREM_transition_prob[1] <- exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2]) /
      (1 + exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2]) + exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1]))
    nonREM_transition_prob[2] <- exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1]) /
      (1 + exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2]) + exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1]))
    nonREM_transition_prob[3] <- 1 /
      (1 + exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2]) + exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1]))
    
    # nonREM, no event
    nonREM_transition_prob_event[1] <- exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2] + fixed_vec_mat[curr_epoch,6] + theta_curr[6]) /
      (1 + exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2] + fixed_vec_mat[curr_epoch,6] + theta_curr[6]) +
         exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1] + fixed_vec_mat[curr_epoch,5] + theta_curr[5]))
    nonREM_transition_prob_event[2] <- exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1] + fixed_vec_mat[curr_epoch,5] + theta_curr[5]) /
      (1 + exp(fixed_vec_mat[curr_epoch,2] + theta_curr[2] + fixed_vec_mat[curr_epoch,6] + theta_curr[6]) +
         exp(fixed_vec_mat[curr_epoch,1] + theta_curr[1] + fixed_vec_mat[curr_epoch,5] + theta_curr[5]))
    ...
    event_rate_REM <- fixed_vec_mat[curr_epoch,9] * exp(theta_curr[9])
    event_rate_nonREM <- fixed_vec_mat[curr_epoch,10] * exp(theta_curr[10])
...
fixed_effect_samples <- matrix(nrow=n_samples_total, ncol=10)
for (i in 1:n_samples_total) {
  fixed_effect_samples[i,] <- c(as.vector(post_samples_event$mu_tau[i,,]),
                                post_samples_event$lambda_R[i],
                                post_samples_event$lambda_N[i])
}
fixed_effect_means <- colMeans(fixed_effect_samples)
true_fixed_effects <- fixed_vec_mean[stan_indices]
fixed_mse <- mean((fixed_effect_means - true_fixed_effects)^2)

### fixed effect coverage
fixed_effect_lower <- apply(fixed_effect_samples, 2, quantile, probs=0.025)
fixed_effect_upper <- apply(fixed_effect_samples, 2, quantile, probs=0.975)
...
############ save results ##################
results_filename <- paste0("scen2_results_seed",
                           random_seed,
                           ".rds")
