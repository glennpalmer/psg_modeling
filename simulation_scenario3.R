###############################################################################
############ Script to run simulation scenario 3 on the server ################
###############################################################################
...
# draw theta_i from mixture of four spherical gaussian distributions
theta_centers <- rmvnorm(n=n_clusters, sigma=diag(10)*0.25)
cluster_labels <- sample(1:n_clusters, n_patients, replace=TRUE)
theta_mat <- matrix(nrow=n_patients, ncol=10)
for (i in 1:n_patients) {
  theta_mat[i,] <- rmvnorm(n=1, mean=theta_centers[cluster_labels[i],], sigma=diag(10)*0.25)
}

# center theta for identifiability
for (j in 1:10) {
  theta_mat[,j] <- theta_mat[,j] - mean(theta_mat[,j])
}
...
stan_indices <- c(4,2,8,6,3,1,7,5,9,10)
...
############ save results ##################
results_filename <- paste0("scen3_results_seed",
                           random_seed,
                           ".rds")
